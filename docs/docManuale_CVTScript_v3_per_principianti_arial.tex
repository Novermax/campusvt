...
\begin{figure}[H]
  \centering
   \includegraphics[width=16cm]{immagini/figura 1.png}
...
\begin{figure}[H]
  \centering
   \includegraphics[width=16cm]{immagini/cap01/figura 1.1.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=16cm]{immagini/cap01/figura 1.2.png}
 %\imgph{12cm}{Screenshot del tutorial in esecuzione con quattro callout numerati: 1=modello 3D al centro, 2=fumetto della spiegazione, 3=barra strumenti in basso, 4=freccia ``avanti'' a destra.}
  \caption{L'interfaccia che vede lo studente:.}
...
\begin{figure}[H]
  \centering
   \includegraphics[width=16cm]{immagini/cap02/figura 2.1.png}
...
\begin{notablu}[Convenzione opzionale per i materiali emissivi]
Se nel modello hai un materiale Blender che rappresenta un elemento
realmente \emph{auto-illuminante} (LED, spia luminosa, display retro\-illuminato)
e vuoi che il sistema ne potenzi la luminosità in scena, puoi nominare
quel materiale con un nome che contenga \texttt{luminoso},
\texttt{emissive} oppure \texttt{glow}: al caricamento il modello
verrà rilevato e impostato a \texttt{emissiveIntensity = 3.0}.

È una convenzione \emph{opzionale} sui nomi dei materiali, non una
direttiva del file \file{cvtscript}, e nessuno degli scenari attuali
la usa. Non confonderla con l'evidenziazione gialla degli elementi
interattivi: quella è gestita automaticamente dal motore in base allo
step e funziona indipendentemente dal nome del materiale.
\end{notablu}

\begin{figure}[H]
  \centering
  \includegraphics[width=16cm]{immagini/cap02/figura 2.2.png}
...
ruota e rispetto al quale vengono calcolate le traslazioni. Per gli
oggetti che devono ruotare attorno a un proprio asse (viti, perni,
alberi), il vincolo importante è che \textbf{l'origine giaccia
sull'asse di rotazione}. Per una vite, qualunque punto lungo l'asse
del gambo va bene: testa, centro o punta sono tutti equivalenti dal
punto di vista del comportamento dell'animazione.

Quello che invece va evitato è un'origine \emph{fuori asse} (spostata
lateralmente rispetto al gambo): in quel caso la vite, durante \kw{do
: unscrew}, ruoterebbe descrivendo un cono invece di girare su sé
stessa.

\begin{trucchetto}
In Blender usa \emph{Object $\rightarrow$ Set Origin $\rightarrow$
Origin to 3D Cursor} dopo aver posizionato il cursore 3D sull'asse
di rotazione dell'oggetto. La testa della vite è una scelta comoda
perché coincide con il punto visivo di presa, ma non è un requisito
tecnico: anche il centro del gambo o la punta funzionano allo stesso
modo, purché stiano sull'asse.
\end{trucchetto}

\begin{figure}[H]
  \centering
  % \includegraphics[width=11cm]{immagini/cap02/origine_vite.png}
  \imgph{11cm}{Confronto a tre viti identiche viste di lato. A sinistra origine spostata lateralmente rispetto al gambo (marcata con una X rossa, didascalia ``fuori asse: sbagliato''), una freccia curva mostra una rotazione conica anomala. Al centro origine sul centro del gambo (punto verde, didascalia ``sull'asse: giusto''). A destra origine sulla testa della vite (punto verde, didascalia ``sull'asse: giusto''). Le due varianti di destra hanno entrambe una freccia di rotazione assiale corretta attorno al gambo.}
  \caption{L'origine deve stare sull'asse di rotazione del gambo. La posizione esatta lungo l'asse (testa, centro, punta) è indifferente; quello che conta è non spostarla lateralmente.}
...
\begin{enumerate}
  ...
  \item[$\square$] L'origine è posizionata sull'asse di rotazione
        dell'oggetto (per le viti: lungo il gambo, non spostata
        lateralmente).
...
\begin{figure}[H]
  ...
  \imgph{12cm}{Checklist a 6 voci con icone simboliche per ognuna: file GLB, lettere minuscole, pivot sull'asse di una vite, gerarchia ad albero, registro \file{homeconfig.ini}, cartella \file{models/}. Ogni voce ha una casella $\square$ a sinistra.}
  \caption{Sei punti da spuntare prima di passare alla scrittura del tutorial.}
