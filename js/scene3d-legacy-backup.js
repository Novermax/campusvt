...
window.Scene3D = {
    ...
    animationSystem: {
        ...
        // NUOVO: Sistema multi-step movimento
        multiStepAnimations: new Map() // Mappa model.uuid -> { steps: [...], currentStep: 0, ... }
    },
    ...
    init: function() {
        ...
        // Inizializza flag per camera manuale
    ...
    fitModelToView: function(model) {
        ...
        // Posiziona la camera SOLO se non è stata impostata manualmente dallo scenario
        if (!this.manualCameraSet) {
            this.camera.position.set(center.x, center.y, center.z + cameraZ);
            this.camera.lookAt(center);
        } else {
            console.log('📷 Auto-fitting singolo modello disabilitato: camera già impostata dallo scenario');
        }
    ...
    fitAllModelsToView: function() {
        ...
        if (!this.manualCameraSet) {
            ...
            console.log('📷 Auto-fitting tutti i modelli disabilitato: camera già impostata dallo scenario');
        }
    ...
    /**
     * NUOVO: Applica impostazioni telecamera da tutorial step
     * @param {Object} tutorialStep - Step del tutorial con parametri camera
     */
    applyCameraSettings: function(tutorialStep) {
        ...
        // NUOVO: Controlla se ci sono effettivamente parametri camera in questo step
        ...
        if (!hasCameraSettings) {
            ...
            return; // Esce senza modificare la camera
        }
        
        // Parsing CameraPos=(x, y, z) o CameraPos=elemento.glb:(x, y, z)
        if (props.CameraPos) {
            let targetPosition = null;
            
            // Controlla se è formato relativo elemento:offset
            if (props.CameraPos.includes(':')) {
                ...
                if (offsetMatch) {
                    ...
                    if (offsetCoords.length === 3) {
                        const elementCenter = this.getElementCenter(elementName);
                        if (elementCenter) {
                            ...
                            console.log(`⚠️ CAMERA: Elemento "${elementName}" non trovato per posizione relativa, uso coordinate assolute`);
                        }
                    }
                }
            } else {
                // Formato coordinate assolute standard (x, y, z)
            ...
            if (targetPosition) {
                cameraChanged = true;
                // Salva posizione target per animazione
                this.cameraAnimation = this.cameraAnimation || {};
                this.cameraAnimation.targetPosition = targetPosition;
            }
        }
        
        // Parsing CameraTarget=(x, y, z) o CameraTarget=elemento.glb
        if (props.CameraTarget) {
            let targetLookAt = null;
            
            // Controlla se è in formato coordinate (x, y, z)
            ...
            if (coordMatch) {
                ...
                if (coords.length === 3) {
                    ...
                    console.log(`📹 CAMERA: Nuovo target coordinate ${targetLookAt.x}, ${targetLookAt.y}, ${targetLookAt.z}`);
                }
            } else {
                // È un nome di elemento, cerca il centro dell'oggetto
                const elementName = props.CameraTarget.trim();
                targetLookAt = this.getElementCenter(elementName);
                if (targetLookAt) {
                    console.log(`📹 CAMERA: Nuovo target elemento "${elementName}" centro=(${targetLookAt.x.toFixed(3)}, ${targetLookAt.y.toFixed(3)}, ${targetLookAt.z.toFixed(3)})`);
                } else {
                    console.log(`⚠️ CAMERA: Elemento "${elementName}" non trovato, uso target corrente`);
                }
            }
        ...
        if (props.CameraZoom) {
            ...
            if (!isNaN(zoom)) {
                console.log(`📹 CAMERA: Nuovo zoom ${zoom}`);
                cameraChanged = true;
        ...
        // La camera usa sempre easing dolce (smootherStep)
        
        // Se c'è almeno un parametro camera, avvia l'animazione
        if (cameraChanged) {
    ...
    /**
     * NUOVO: Trova il centro di un elemento di scena per nome
     * @param {string} elementName - Nome dell'elemento (es: "vite_coperchio_1.glb")
     * @returns {THREE.Vector3|null} - Centro dell'elemento o null se non trovato
     */
    getElementCenter: function(elementName) {
        if (!elementName) return null;
        
        console.log(`🔍 DEBUG: Cercando elemento "${elementName}" tra ${this.loadedModels.length} modelli`);
        
        // DEBUG: Lista tutti i modelli caricati per troubleshooting
        console.log(`🔍 DEBUG: Modelli caricati:`);
        this.loadedModels.forEach((model, index) => {
            const filename = model.userData?.originalFilename || model.name;
            console.log(`  [${index}] Nome: "${model.name}", Filename: "${filename}"`);
        });
        
        // Cerca nell'array dei modelli caricati
        const targetModel = this.loadedModels.find(model => {
            const filename = model.userData?.originalFilename || model.name;
        ...
        if (targetModel) {
            // Calcola il centro usando bounding box
            const boundingBox = new THREE.Box3().setFromObject(targetModel);
            const center = boundingBox.getCenter(new THREE.Vector3());
            
            console.log(`🎯 ELEMENTO: "${elementName}" trovato, centro=(${center.x.toFixed(3)}, ${center.y.toFixed(3)}, ${center.z.toFixed(3)})`);
            return center;
        }
        
        // Se non trovato nei modelli caricati, cerca nella scena generale
        let foundObject = null;
        this.scene.traverse((object) => {
            if (foundObject) return; // Già trovato
        ...
        if (foundObject) {
            ...
            console.log(`🎯 SCENA: "${elementName}" trovato nella scena, centro=(${center.x.toFixed(3)}, ${center.y.toFixed(3)}, ${center.z.toFixed(3)})`);
            return center;
        }
        
        console.log(`❌ ELEMENTO: "${elementName}" non trovato né nei modelli caricati né nella scena`);
        return null;
    },
    
    /**
     * NUOVO: Anima la telecamera verso la posizione target
     * @param {number} duration - Durata animazione in secondi
     */
    animateCamera: function(duration) {
        if (!this.cameraAnimation) return;
        
        const startPosition = this.camera.position.clone();
        const startLookAt = this.camera.getWorldDirection(new THREE.Vector3()).multiplyScalar(-1).add(this.camera.position);
        const startZoom = this.camera.zoom || 1.0;
        ...
        const animation = {
            ...
            initialLookAt: startLookAt,
            initialZoom: startZoom,
            
            // Posizioni target
            targetPosition: this.cameraAnimation.targetPosition || startPosition,
            targetLookAt: this.cameraAnimation.targetLookAt || startLookAt,
            targetZoom: this.cameraAnimation.targetZoom || startZoom,
        ...
        console.log(`📹 CAMERA: Animazione avviata (durata: ${duration}s)`);
        console.log(`📹 Da: pos(${startPosition.x.toFixed(2)}, ${startPosition.y.toFixed(2)}, ${startPosition.z.toFixed(2)})`);
        console.log(`📹 A: pos(${animation.targetPosition.x.toFixed(2)}, ${animation.targetPosition.y.toFixed(2)}, ${animation.targetPosition.z.toFixed(2)})`);
        
        // Pulisci i dati di animazione
        this.cameraAnimation = null;
    },
    
    /**
     * NUOVO: Aggiorna animazione telecamera
     * @param {Object} anim - Oggetto animazione telecamera
     * @param {number} progress - Progresso da 0 a 1
     */
    updateCameraAnimation: function(anim, progress) {
        // Interpola posizione telecamera
        if (anim.targetPosition) {
            this.camera.position.lerpVectors(anim.initialPosition, anim.targetPosition, progress);
        }
        
        // Interpola target di visualizzazione (lookAt)
        if (anim.targetLookAt) {
            const currentLookAt = new THREE.Vector3().lerpVectors(anim.initialLookAt, anim.targetLookAt, progress);
            this.camera.lookAt(currentLookAt);
            
            // Aggiorna anche il pivot per i controlli mouse
            this.mouseControls.pivotPoint.copy(currentLookAt);
        }
        
        // Interpola zoom
        if (anim.targetZoom !== undefined) {
            const currentZoom = anim.initialZoom + (anim.targetZoom - anim.initialZoom) * progress;
            this.camera.zoom = currentZoom;
            this.camera.updateProjectionMatrix();
        }
        
        // Debug occasionale
        if (progress === 0 || progress === 1 || Math.random() < 0.02) {
            console.log(`📹 CAMERA: progress=${progress.toFixed(3)}, pos=(${this.camera.position.x.toFixed(2)}, ${this.camera.position.y.toFixed(2)}, ${this.camera.position.z.toFixed(2)}), zoom=${this.camera.zoom.toFixed(2)}`);
        }
    },
    ...
    startModelAnimation: function(model, tutorialStep) {
        ...
        // NUOVO: Applica impostazioni telecamera se presenti
        this.applyCameraSettings(tutorialStep);
        
        // NUOVO: Controlla se ci sono sub-steps di movimento definiti
        const movementSteps = this.parseMovementSteps(tutorialStep);
        if (movementSteps.length > 0) {
            console.log(`🎬 MULTI-STEP: Trovati ${movementSteps.length} sub-steps, avvio sequenza multi-step`);
            return this.startMultiStepMovement(model, movementSteps);
        }
        
        // Altrimenti usa il sistema di animazione tradizionale
        console.log(`🎬 LEGACY: Nessun sub-step trovato, uso animazione tradizionale`);
        ...
        // Durata con supporto override da tutorial.txt
        ...
        if (customDuration) {
            console.log(`⏱️ DURATA: Override personalizzato ${customDuration}s (default era ${defaultDuration}s)`);
        } else {
            console.log(`⏱️ DURATA: Default ${duration}s per azione ${action}`);
        }
    ...
    /**
     * NUOVO: Applica rotazione attorno a un centro personalizzato (coordinate assolute)
     * @param {Object} anim - Oggetto animazione con rotationCenter assoluto
     * @param {number} progress - Progresso da 0 a 1
     */
    applyRotationAroundCustomCenter: function(anim, progress) {
        // Calcola rotazione corrente interpolata
        const currentRotation = new THREE.Euler(
            anim.initialRotation.x + (anim.targetRotation.x - anim.initialRotation.x) * progress,
            anim.initialRotation.y + (anim.targetRotation.y - anim.initialRotation.y) * progress,
            anim.initialRotation.z + (anim.targetRotation.z - anim.initialRotation.z) * progress
        ...
        // Calcola movimento lineare di traslazione (se presente)
        const linearMovement = new THREE.Vector3().lerpVectors(anim.initialPosition, anim.targetPosition, progress);
        
        // Rotazione attorno al centro personalizzato usando matrici
        const pivotCenter = anim.rotationCenter; // Centro in coordinate assolute
        
        // Crea la matrice di trasformazione completa
        const matrix = new THREE.Matrix4();
        
        // 1. Trasla al centro di rotazione (porta il centro all'origine)
        matrix.makeTranslation(-pivotCenter.x, -pivotCenter.y, -pivotCenter.z);
        
        // 2. Applica la rotazione relativa dalla rotazione iniziale
        ...
        // 3. Trasla indietro dal centro di rotazione
        const translateBack = new THREE.Matrix4();
        translateBack.makeTranslation(pivotCenter.x, pivotCenter.y, pivotCenter.z);
        matrix.premultiply(translateBack);
        
        // 4. Calcola la nuova posizione applicando la rotazione alla posizione iniziale
        const rotatedPosition = anim.initialPosition.clone();
        rotatedPosition.applyMatrix4(matrix);
        
        // 5. Combina rotazione + traslazione lineare
        // Se c'è anche traslazione, aggiungila alla posizione ruotata
        ...
        // 6. Applica la trasformazione al modello
        anim.model.position.copy(rotatedPosition);
        anim.model.rotation.copy(currentRotation);
        
        // Debug ogni tanto - Enhanced for Step 9
        if (progress === 0 || progress === 1 || Math.random() < 0.1) {
            const modelName = anim.model.userData?.originalFilename || anim.model.name;
            console.log(`🎯 ROTAZIONE PERSONALIZZATA [${modelName}]: progress=${progress.toFixed(3)}`);
            console.log(`   Pivot center: (${pivotCenter.x.toFixed(3)}, ${pivotCenter.y.toFixed(3)}, ${pivotCenter.z.toFixed(3)})`);
            console.log(`   Initial pos: (${anim.initialPosition.x.toFixed(3)}, ${anim.initialPosition.y.toFixed(3)}, ${anim.initialPosition.z.toFixed(3)})`);
            console.log(`   Target pos: (${anim.targetPosition.x.toFixed(3)}, ${anim.targetPosition.y.toFixed(3)}, ${anim.targetPosition.z.toFixed(3)})`);
            console.log(`   Current rotation: (${currentRotation.x.toFixed(3)}, ${currentRotation.y.toFixed(3)}, ${currentRotation.z.toFixed(3)})`);
            console.log(`   Rotated position: (${rotatedPosition.x.toFixed(3)}, ${rotatedPosition.y.toFixed(3)}, ${rotatedPosition.z.toFixed(3)})`);
            console.log(`   Delta rotation: (${deltaRotation.x.toFixed(3)}, ${deltaRotation.y.toFixed(3)}, ${deltaRotation.z.toFixed(3)})`);
        }
    },
    ...
    updateAnimations: function() {
        ...
        for (let i = this.animationSystem.activeAnimations.length - 1; i >= 0; i--) {
            ...
            // NUOVO: Gestione animazione telecamera (duration in ms)
            if (anim.isCamera) {
                progress = Math.min(elapsed / (anim.duration / 1000), 1.0);
                // Usa sempre easing dolce per la camera
                progress = this.smootherStep(progress);
                this.updateCameraAnimation(anim, progress);
            } 
            // Animazione modelli (duration in secondi)
            else {
                ...
                if (anim.targetRotation) {
                    // Debug for Step 9 rotation issue
                    const modelName = anim.model.userData?.originalFilename || anim.model.name;
                    if (modelName && modelName.includes('ingrassatore')) {
                        console.log(`🔍 STEP 9 CHECK [${modelName}]: isMultiStep=${anim.isMultiStep}, hasRotationCenter=${!!anim.rotationCenter}`);
                        if (anim.rotationCenter) {
                            console.log(`🔍 STEP 9 CENTER: (${anim.rotationCenter.x.toFixed(3)}, ${anim.rotationCenter.y.toFixed(3)}, ${anim.rotationCenter.z.toFixed(3)})`);
                        }
                    }
                    
                    // NUOVO: Multi-step con centro personalizzato
                    if (anim.isMultiStep && anim.rotationCenter) {
                        this.applyRotationAroundCustomCenter(anim, progress);
                        
                        // Debug per multi-step
                        if (Math.floor(currentTime / 100) !== Math.floor(anim.lastDebugTime || 0 / 100)) {
                            anim.lastDebugTime = currentTime;
                            const modelName = anim.model.userData?.originalFilename || anim.model.name;
                            console.log(`🎯 MULTI-STEP ${modelName}: progress=${progress.toFixed(2)}, centro personalizzato`);
                        }
                    }
                    // Per azioni con rotazione (svita/avvita) usiamo rotazione attorno al centro geometrico
                        ...
                        // Debug for Step 9 fallback
                        if (modelName && modelName.includes('ingrassatore')) {
                            console.log(`🔍 STEP 9 FALLBACK [${modelName}]: Using basic slerp rotation (no custom center)`);
                            console.log(`🔍 STEP 9 FALLBACK: action="${anim.action}", isMultiStep=${anim.isMultiStep}`);
                        }
            ...
            if (progress >= 1.0) {
                ...
                // NUOVO: Gestione speciale per animazioni camera (non hanno model)
                if (anim.isCamera) {
                    console.log('📹 CAMERA: Animazione camera completata');
                } else {
                    // Animazioni di modelli
                    ...
                    // NUOVO: Gestione multi-step vs animazione singola
                    if (anim.isMultiStep) {
                        // È un sub-step di una sequenza multi-step
                        this.onMultiStepCompleted(anim.modelUuid);
                    } else {
                        // Animazione singola tradizionale
    ...
    /**
     * Funzione di easing più dolce per camera (accelerazione e decelerazione molto graduali)
     * Implementa la curva smootherstep: 6t⁵ - 15t⁴ + 10t³
     * Fornisce transizioni più fluide con accelerazione/decelerazione più morbide
     */
    smootherStep: function(t) {
        // Clamp tra 0 e 1
        t = Math.max(0, Math.min(1, t));
        
        // Smoother step formula (Ken Perlin)
    ...
    /* ===== SISTEMA MULTI-STEP MOVIMENTO ===== */
    
    /**
     * Parsea i sub-steps di movimento da un tutorial step
     * @param {Object} tutorialStep - Lo step del tutorial
     * @returns {Array} - Array di sub-steps di movimento
     */
    parseMovementSteps: function(tutorialStep) {
        const movementSteps = [];
        let stepIndex = 1;
        ...
        console.log(`🎬 MULTI-STEP: Totale ${movementSteps.length} sub-steps trovati`);
        return movementSteps;
    },
    
    /**
     * Parsea una stringa di movimento step
     * Formato: "rotazione:(x,y,z,durata);traslazione:(x,y,z,durata)"
     * @param {string} stepString - La stringa da parsare
     * @param {number} stepIndex - Indice dello step per debug
     * @returns {Object|null} - Oggetto step parsato o null se errore
     */
    parseMovementStepString: function(stepString, stepIndex) {
        try {
            const step = {
                index: stepIndex,
                rotazione: null,
                traslazione: null,
                centro: null,        // NUOVO: centro di rotazione relativo
                durata: 1.0 // Default 1 secondo
            };
            
            // Split per operazioni separate da ;
            ...
            // La durata è il massimo tra rotazione e traslazione (per parallelismo)
            ...
            console.error(`❌ Errore parsing MovimentoStep${stepIndex}:`, error);
            return null;
        }
    },
    
    /**
     * Parsea una singola operazione di movimento
     * @param {string} operationString - Es: "rotazione:(90,0,0,2.0)" o "traslazione:target.glb,(0,0,0,1.0)"
     * @param {string} type - "rotazione", "traslazione" o "centro"
     * @returns {Object} - { x, y, z, durata } o { x, y, z } per centro, con targetElement se specificato
     */
    parseMovementOperation: function(operationString, type) {
        // NUOVO: Gestione traslazione verso elemento target
        if (type === 'traslazione' && operationString.includes(':') && !operationString.match(/^traslazione:\(/)) {
            // Formato: "traslazione:target.glb" o "traslazione:target.glb,(-0.1,0,0,1.5)"
            ...
            if (afterColon.includes(',')) {
                // Ha offset: "target.glb,(-0.1,0,0,1.5)"
                ...
                // Solo target, senza offset: "target.glb" 
                targetElement = afterColon.trim();
                offsetValues = [0, 0, 0, 1.0]; // Default: nessun offset, durata 1s
            }
            
            console.log(`🎯 TRASLAZIONE: Verso elemento "${targetElement}" con offset (${offsetValues[0]}, ${offsetValues[1]}, ${offsetValues[2]}) durata ${offsetValues[3]}s`);
            ...
            return {
                ...
                targetElement: targetElement  // NUOVO: elemento target per traslazione
            };
        }
        
        // Gestione standard per rotazione, centro, e traslazione relativa
        ...
        // Il comando centro ha solo 3 parametri (x,y,z relativi), senza durata
        if (type === 'centro') {
            if (values.length !== 3) {
            ...
            // Rotazione e traslazione hanno 4 parametri (x,y,z,durata)
            if (values.length !== 4) {
    ...
    /**
     * Calcola il centro di rotazione assoluto basato su coordinate relative all'oggetto
     * @param {THREE.Object3D} model - Il modello di riferimento
     * @param {Object} relativeCenter - Coordinate relative {x, y, z} dove (0,0,0) è il centro dell'oggetto
     * @returns {THREE.Vector3} - Centro di rotazione in coordinate assolute
     */
    calculateRelativeRotationCenter: function(model, relativeCenter) {
        if (!relativeCenter) {
            // Se non specificato, usa il centro dell'oggetto
            const box = new THREE.Box3().setFromObject(model);
        ...
        // Converti coordinate relative in offset assoluto
        // relativeCenter.x = 0 → centro, relativeCenter.x = 1 → bordo destro, ecc.
        ...
        console.log(`🎯 CENTRO: Oggetto center=(${objectCenter.x.toFixed(3)}, ${objectCenter.y.toFixed(3)}, ${objectCenter.z.toFixed(3)})`);
        console.log(`🎯 CENTRO: Oggetto size=(${objectSize.x.toFixed(3)}, ${objectSize.y.toFixed(3)}, ${objectSize.z.toFixed(3)})`);
        console.log(`🎯 CENTRO: Relativo=(${relativeCenter.x}, ${relativeCenter.y}, ${relativeCenter.z})`);
        console.log(`🎯 CENTRO: Assoluto=(${absoluteCenter.x.toFixed(3)}, ${absoluteCenter.y.toFixed(3)}, ${absoluteCenter.z.toFixed(3)})`);
        
        return absoluteCenter;
    },
    
    /**
     * Avvia una sequenza di multi-step movimento per un modello
     * @param {THREE.Object3D} model - Il modello da animare
     * @param {Array} movementSteps - Array di step di movimento
     */
    startMultiStepMovement: function(model, movementSteps) {
        if (!movementSteps || movementSteps.length === 0) {
            console.log('🎬 MULTI-STEP: Nessun sub-step di movimento definito');
            return false;
        }
        ...
        console.log(`🎬 MULTI-STEP: Avvio sequenza ${movementSteps.length} step per ${modelName}`);
        
        // Salva la sequenza nella mappa
    ...
    /**
     * Esegue lo step corrente di un multi-step movimento
    ...
    executeCurrentMultiStep: function(modelUuid) {
        ...
        if (!currentStep) {
            console.log(`🎬 MULTI-STEP: Sequenza completata per ${multiStepData.modelName}`);
            this.finishMultiStepMovement(modelUuid);
            return;
        }
        
        console.log(`🎬 MULTI-STEP: Esecuzione step ${multiStepData.currentStepIndex + 1}/${multiStepData.steps.length} per ${multiStepData.modelName}`);
        console.log(`🎬 MULTI-STEP: Step data:`, currentStep);
        
        const model = multiStepData.model;
        
        // Calcola posizioni/rotazioni iniziali e finali
        ...
        // Applica traslazione se presente
        if (currentStep.traslazione) {
            // NUOVO: Traslazione verso elemento target
            if (currentStep.traslazione.targetElement) {
                const targetCenter = this.getElementCenter(currentStep.traslazione.targetElement);
                if (targetCenter) {
                    // DEBUG: Log posizioni per troubleshooting allineamento
                    console.log(`🎯 ALLINEAMENTO DEBUG: Elemento che si muove: "${model.userData?.originalFilename || model.name}"`);
                    console.log(`🎯 ALLINEAMENTO DEBUG: Posizione corrente ingrassatore: (${model.position.x.toFixed(3)}, ${model.position.y.toFixed(3)}, ${model.position.z.toFixed(3)})`);
                    console.log(`🎯 ALLINEAMENTO DEBUG: Centro target "${currentStep.traslazione.targetElement}": (${targetCenter.x.toFixed(3)}, ${targetCenter.y.toFixed(3)}, ${targetCenter.z.toFixed(3)})`);
                    console.log(`🎯 ALLINEAMENTO DEBUG: Offset richiesto: (${currentStep.traslazione.x.toFixed(3)}, ${currentStep.traslazione.y.toFixed(3)}, ${currentStep.traslazione.z.toFixed(3)})`);
                    
                    // Muove verso il centro del target + offset
                    targetPosition = new THREE.Vector3(
                        targetCenter.x + currentStep.traslazione.x,
                        targetCenter.y + currentStep.traslazione.y,
                        targetCenter.z + currentStep.traslazione.z
                    );
                    console.log(`🎯 ALLINEAMENTO DEBUG: Posizione finale calcolata: (${targetPosition.x.toFixed(3)}, ${targetPosition.y.toFixed(3)}, ${targetPosition.z.toFixed(3)})`);
                    
                    // Calcola la distanza per troubleshooting
                    const distance = model.position.distanceTo(targetPosition);
                    console.log(`🎯 ALLINEAMENTO DEBUG: Distanza da percorrere: ${distance.toFixed(3)} unità`);
                } else {
                    console.log(`⚠️ MULTI-STEP: Target "${currentStep.traslazione.targetElement}" non trovato, uso movimento relativo`);
                    // Fallback: movimento relativo normale
        ...
        // NUOVO: Calcola centro di rotazione (se specificato)
        let rotationCenter = null;
        if (currentStep.rotazione && currentStep.centro) {
            console.log(`🎯 STEP 9 DEBUG: About to calculate rotation center`);
            console.log(`🎯 STEP 9 DEBUG: Model name: ${model.userData?.originalFilename || model.name}`);
            console.log(`🎯 STEP 9 DEBUG: Model position: (${model.position.x.toFixed(3)}, ${model.position.y.toFixed(3)}, ${model.position.z.toFixed(3)})`);
            console.log(`🎯 STEP 9 DEBUG: Relative center: (${currentStep.centro.x}, ${currentStep.centro.y}, ${currentStep.centro.z})`);
            console.log(`🎯 STEP 9 DEBUG: Rotation: (${currentStep.rotazione.x}°, ${currentStep.rotazione.y}°, ${currentStep.rotazione.z}°)`);
            rotationCenter = this.calculateRelativeRotationCenter(model, currentStep.centro);
            console.log(`🎯 STEP 9 DEBUG: Calculated absolute rotation center: (${rotationCenter.x.toFixed(3)}, ${rotationCenter.y.toFixed(3)}, ${rotationCenter.z.toFixed(3)})`);
        }
        
        // Applica rotazione se presente (in gradi -> radianti)
        ...
        // Crea l'animazione con durata del step
        const animation = {
            model: model,
            modelUuid: modelUuid,
            ...
            rotationCenter: rotationCenter,        // NUOVO: Centro di rotazione personalizzato
        ...
        console.log(`🎬 MULTI-STEP: Animazione step ${multiStepData.currentStepIndex + 1} avviata (durata: ${currentStep.durata}s)`);
    },
    
    /**
     * Chiamata quando uno step di multi-movimento è completato
     * @param {string} modelUuid - UUID del modello
     */
    onMultiStepCompleted: function(modelUuid) {
        ...
        console.log(`🎬 MULTI-STEP: Step ${multiStepData.currentStepIndex}/${multiStepData.steps.length} completato per ${multiStepData.modelName}`);
        
        // Piccola pausa prima del prossimo step (opzionale)
        setTimeout(() => {
            this.executeCurrentMultiStep(modelUuid);
        }, 100);
    },
    ...
    /**
     * Termina una sequenza multi-step
     * @param {string} modelUuid - UUID del modello
     */
    finishMultiStepMovement: function(modelUuid) {
        ...
        if (multiStepData) {
            ...
            // Marca il tutorial step come completato
            if (window.UI && window.UI.currentStepIndex !== undefined && window.UI.currentStepIndex >= 0) {
                this.markStepAsCompleted(window.UI.currentStepIndex);
            }
            
            // Auto-avanza al prossimo step del tutorial
            this.advanceToNextTutorialStep();
        }
    },
    ...
    /**
     * Test per il parsing dei sub-steps di movimento
     */
    testMultiStepParsing: function() {
        console.log('🎬 === TEST PARSING MULTI-STEP ===');
        
        // Test con vari formati
        const testCases = [
            'rotazione:(90,0,0,2.0);traslazione:(0,0.5,0,1.5)',
            'traslazione:(0.3,0,0.2,1.0)',
            'rotazione:(-90,180,0,3.0)',
            'traslazione:target_elemento.glb',
            'traslazione:target_elemento.glb,(-0.1,0.2,0,1.5)',
            'centro:(0,-0.5,0);rotazione:(45,0,0,1.0);traslazione:elemento.glb,(0.1,0,0,1.0)'
        ];
        
        testCases.forEach((testString, index) => {
            console.log(`🎬 TEST ${index + 1}: "${testString}"`);
            try {
                const parsed = this.parseMovementStepString(testString, index + 1);
    ...
    /**
     * Crea un tutorial step di test con multi-step movimento
     */
    createTestMultiStepTutorial: function() {
        return {
            title: "Test Multi-Step",
            properties: {
                Elemento: "vite_coperchio_1.glb",
                Utensile: "Mani", 
                Descrizione: "Test movimento complesso multi-step",
                MovimentoStep1: "rotazione:(90,0,0,1.0);traslazione:(0,0.2,0,1.0)",
                MovimentoStep2: "rotazione:(0,-180,0,1.5);traslazione:(0.3,0,0,1.5)",
                MovimentoStep3: "traslazione:(0,0.5,0,2.0)"
            }
        };
    },
    
    /**
     * NUOVO: Crea tutorial di test per centro di rotazione personalizzato
     */
    createTestCustomCenterTutorial: function() {
        return {
            title: "Test Centro Personalizzato",
            properties: {
                Elemento: "ingrassatore.glb",
                Utensile: "Mani",
                Descrizione: "Test rotazione con centro personalizzato (ingrassatore)",
                MovimentoStep1: "centro:(0,-0.8,0);rotazione:(0,-15,0,1.0)",
                MovimentoStep2: "centro:(0,-0.8,0);rotazione:(0,-90,0,1.5)", 
                MovimentoStep3: "traslazione:(0,0.3,0,1.2)"
            }
        };
    },
    
    /**
     * NUOVO: Crea tutorial di test per traslazione verso elementi target
     */
    createTestTargetTranslationTutorial: function() {
        return {
            title: "Test Traslazione Target",
            properties: {
                Elemento: "ingrassatore.glb",
                Utensile: "Mani",
                Descrizione: "Test allineamento verso elemento target",
                MovimentoStep1: "traslazione:(0,0.1,0,0.8)",
                MovimentoStep2: "traslazione:tappino_grasso_sx.glb,(-0.1,0,0,1.5)",
                MovimentoStep3: "traslazione:coperchio.glb,(0,0.2,0,2.0)"
            }
        };
    },
    
    /**
     * Test manuale del sistema multi-step
     */
    testMultiStepAnimation: function() {
        console.log('🎬 === TEST ANIMAZIONE MULTI-STEP ===');
        
        if (this.loadedModels.length === 0) {
            console.log('❌ Nessun modello caricato per il test');
            return;
        }
        
        // Usa il primo modello disponibile
        const testModel = this.loadedModels[0];
        const testStep = this.createTestMultiStepTutorial();
        
        console.log('🎬 Modello test:', testModel.userData?.originalFilename || testModel.name);
        console.log('🎬 Step test:', testStep);
        
        // Avvia l'animazione multi-step
        this.startModelAnimation(testModel, testStep);
        
        console.log('🎬 Test multi-step avviato! Guarda la scena per vedere l\'animazione.');
    },
    
    /**
     * NUOVO: Test del sistema di centro di rotazione personalizzato
     */
    testCustomCenterAnimation: function() {
        console.log('🎯 === TEST CENTRO ROTAZIONE PERSONALIZZATO ===');
        ...
        // Cerca un ingrassatore se disponibile, altrimenti usa il primo modello
        let testModel = this.loadedModels.find(m => 
            (m.userData?.originalFilename || m.name).toLowerCase().includes('ingrassatore')
        );
        
        if (!testModel) {
            testModel = this.loadedModels[0];
            console.log('⚠️ Ingrassatore non trovato, uso primo modello disponibile');
        }
        
        const testStep = this.createTestCustomCenterTutorial();
        
        console.log('🎯 Modello test:', testModel.userData?.originalFilename || testModel.name);
        console.log('🎯 Step test con centro personalizzato:', testStep.properties);
        
        // Mostra informazioni sul modello
        const box = new THREE.Box3().setFromObject(testModel);
        const center = box.getCenter(new THREE.Vector3());
        const size = box.getSize(new THREE.Vector3());
        
        console.log('📦 Bounding box modello:');
        console.log(`   Centro: (${center.x.toFixed(3)}, ${center.y.toFixed(3)}, ${center.z.toFixed(3)})`);
        console.log(`   Dimensioni: (${size.x.toFixed(3)}, ${size.y.toFixed(3)}, ${size.z.toFixed(3)})`);
        
        // Calcola dove sarà il centro di rotazione per il primo step
        const relativeCenter = { x: 0, y: -0.8, z: 0 };
        const absoluteCenter = this.calculateRelativeRotationCenter(testModel, relativeCenter);
        console.log(`🎯 Centro rotazione calcolato: (${absoluteCenter.x.toFixed(3)}, ${absoluteCenter.y.toFixed(3)}, ${absoluteCenter.z.toFixed(3)})`);
        
        // Avvia l'animazione
        this.startModelAnimation(testModel, testStep);
        
        console.log('🎯 Test centro personalizzato avviato! Osserva la rotazione attorno al punto specificato.');
    },
    
    /**
     * NUOVO: Test del sistema di traslazione verso elementi target
     */
    testTargetTranslationAnimation: function() {
        console.log('🎯 === TEST TRASLAZIONE VERSO TARGET ===');
        ...
        // Usa il primo modello disponibile (preferibilmente ingrassatore)
        let testModel = this.loadedModels.find(m => 
            (m.userData?.originalFilename || m.name).toLowerCase().includes('ingrassatore')
        );
        ...
        const testStep = this.createTestTargetTranslationTutorial();
        
        console.log('🎯 Modello test:', testModel.userData?.originalFilename || testModel.name);
        console.log('🎯 Step test traslazione verso target:', testStep.properties);
        
        // Mostra posizione iniziale del modello
        console.log(`📦 Posizione iniziale modello: (${testModel.position.x.toFixed(3)}, ${testModel.position.y.toFixed(3)}, ${testModel.position.z.toFixed(3)})`);
        
        // Verifica che gli elementi target esistano
        const targets = ['tappino_grasso_sx.glb', 'coperchio.glb'];
        targets.forEach(targetName => {
            const center = this.getElementCenter(targetName);
            if (center) {
                console.log(`✅ Target "${targetName}" trovato: (${center.x.toFixed(3)}, ${center.y.toFixed(3)}, ${center.z.toFixed(3)})`);
            } else {
                console.log(`❌ Target "${targetName}" non trovato`);
            }
        });
        ...
        console.log('🎯 Test traslazione verso target avviato! Osserva il movimento verso gli elementi specificati.');
    },
    
    /**
     * Mostra stato corrente delle animazioni multi-step
     */
    debugMultiStepAnimations: function() {
        console.log('🎬 === DEBUG MULTI-STEP ATTIVI ===');
        console.log('🎬 Animazioni multi-step attive:', this.animationSystem.multiStepAnimations.size);
        
        this.animationSystem.multiStepAnimations.forEach((data, uuid) => {
            console.log(`🎬 ${data.modelName} (${uuid}):`);
            console.log(`   Step: ${data.currentStepIndex + 1}/${data.steps.length}`);
            console.log(`   Attivo: ${data.isActive}`);
            console.log(`   Steps:`, data.steps.map(s => `${s.index}: ${s.durata}s`));
        });
        
        console.log('🎬 Animazioni singole attive:', this.animationSystem.activeAnimations.length);
        
        return {
            multiStepCount: this.animationSystem.multiStepAnimations.size,
            singleAnimationCount: this.animationSystem.activeAnimations.length
        };
    }
};
